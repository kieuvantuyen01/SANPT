...
def encode_problem_es3(tasks, resources):
    ...
    for i in range(len(tasks)):
        for t in range(tasks[i][2]):
            s[i, t] = solver.IntVar(0, 1, f's_{i}_{t}')
    ...
    for i in range(len(tasks)):
        solver.Add(sum(s[i, t] for t in range(tasks[i][0], tasks[i][2] - tasks[i][1] + 1)) == 1)

    # Link s[i][t] and z[i][t]
    for i in range(len(tasks)):
        solver.Add(s[i, tasks[i][0]] == z[i, tasks[i][0]])
        
        for t in range(tasks[i][0], tasks[i][2] - tasks[i][1]):
            solver.Add(s[i, t+1] <= 1 - z[i, t] + z[i, t+1])
            solver.Add(s[i, t+1] <= z[i, t+1])
            solver.Add(s[i, t+1] >= z[i, t+1] - z[i, t])
    ...
    return solver, u, z, s, y

def validate_solution(tasks, solver, u, z, s, y, resources):
    task_resource = {}
    task_times = {}
...
def solve_es3(tasks, resources):
    solver, u, z, s, y = encode_problem_es3(tasks, resources)
    if not solver:
        return "ERROR", 0, 0, 0
    ...
    if status == pywraplp.Solver.OPTIMAL or status == pywraplp.Solver.FEASIBLE:
        ...
        for i in range(len(tasks)):
            ...
            for t in range(tasks[i][0], tasks[i][2] - tasks[i][1] + 1):
                if s[i, t].solution_value() > 0.5:
                    print_to_console_and_log(f"Task {i+1} starts non-preemptive access at time {t}")
        if not validate_solution(tasks, solver, u, z, s, y, resources):
            sys.exit(1)
