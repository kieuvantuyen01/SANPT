...
def encode_problem_es3(tasks, resources):
    ...
    z = [[t + i * max_time + 1 + len(tasks) * resources for t in range(max_time)] for i in range(len(tasks))]

    # Variables d[i][j][t] for non-preemptive access of resource j by task i starting at time t
    d = [[[t + j * max_time + i * max_time * resources + len(tasks) * (resources + max_time) + 1 
    ...
    for i in range(len(tasks)):
        cnf.append([d[i][j][t] for j in range(resources) for t in range(max_time)])
    ...
    for i in range(len(tasks)):
        for j in range(resources):
            for t in range(max_time):
                ...
                for tp in range(t + tasks[i][1], max_time):
                    cnf.append([-d[i][j][t], -z[i][tp]])

    return cnf, max_time, u, z, d

# Use the encoding function with a SAT solver
tasks = [(0, 2, 2), (0, 2, 3)]  # Each task is a tuple (r_i, e_i, d_i)
resources = 2
